...
public class Product extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/mysql_database?useSSL=false","root","");
			
			Statement st = con.createStatement();
			st.executeUpdate("use products");
			
			int res = st.executeUpdate("insert into pets.product(id,name,color,price)values('"+id_int+"','"+name+"','"+color+"','"+price_cost+"')");
			ResultSet rs = st.executeQuery("select * from products.product");
